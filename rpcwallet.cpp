...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <piecoinaddress> <amount> [comment] [comment-to] [note]\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <topiecoinaddress> <amount> [minconf=1] [note] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Piecoin stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid Piecoin stealth address."));
        return result;
    };
...
Value getwalletinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            " \"balance\": xxxxxxx, (numeric) the total piecoin balance of the wallet\n"
...
Value addstakeout(const Array &params, bool fHelp)
{
    ...
    if (!address.IsValid())
    {
        throw runtime_error("Please enter a valid Piecoin address.");
    }
    ...
    string success = "\n Successfully written " + name + ", Piecoin Address:" + a + " side-stake " + sPercent + "% to stake.dat!";
    return success;
...
Value delstakeout(const Array &params, bool fHelp)
{
    ...
    string success = "\n Successfully deleted " + xName + ", Piecoin Address:" + a + " side-stake " + xPercent + "% from stake.dat.";
