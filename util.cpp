...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Pie
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Pie
    // Mac: ~/Library/Application Support/Pie
    // Unix: ~/.pie2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "pie2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "pieconf.txt"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;

    if (!boost::filesystem::exists(pathConfigFile))
    {
        boost::filesystem::path pathConfigFile2("piecoin.conf");
    ...
    if (!fDone)
    {
        ...
        if (!fMatch)
        {
            ...
            string strMessage = _(
                ...
                "If your clock is wrong Piecoin will not work properly."
            );
