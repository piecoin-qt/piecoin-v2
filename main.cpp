...
unsigned int nTargetSpacing     = 180;               // 3 Minutes
...
unsigned int nTimeV221 = 1653240500; // Now  Sun May 22 2022 17:28:20
unsigned int nTimeV231 = 3231178095; // Future  Sun May 22 2072 21:28:15
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight >= nHalvingPoint * YEARLY_BLOCKCOUNT)
    {
        nHalving = nHeight / nHalvingPoint / YEARLY_BLOCKCOUNT;
        nSubsidy = (1024 * COIN) >> nHalving;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Piecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "No plague!";
        CTransaction txNew;
        txNew.nTime = 1653240500;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
