...
int CompareBipiedian(const unsigned char *c1, size_t c1len, const unsigned char *c2, size_t c2len) {
    while (c1len > c2len) {
        if (*c1)
            return 1;
        c1++;
...
bool CKey::CheckSignatureElement(const unsigned char *vch, int len, bool half) {
    return CompareBipiedian(vch, len, vchZero, 0) > 0 &&
           CompareBipiedian(vch, len, half ? vchMaxModHalfOrder : vchMaxModOrder, 32) <= 0;
}
